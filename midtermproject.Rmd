...
nsysu_comment =fread("data/comment_nsysu.csv",encoding = 'UTF-8') 
nuk_comment =fread("data/comment_nuk.csv",encoding = 'UTF-8') 
ptt_comment =fread("data/comment_ptt.csv",encoding = 'UTF-8') 
exam_comment =fread("data/comment_exam.csv",encoding = 'UTF-8') 
```
...
```{r}
all_dates <- 
  data.frame(seq(as.Date(min(data$artDate)), as.Date(max(data$artDate)),by="day"))
names(all_dates) <- c("artDate")
...
data_count_by_date <- data %>% 
  ...
  summarise(count = n()) %>% 
  arrange(desc(count))
data_count_by_date_plot <- all_dates %>% 
  merge(data_count_by_date,by.x=c('artDate'),by.y=c('artDate'),
        all.x=T,all.y=T) %>% 
  mutate(count = replace_na(count, 0)) %>% 
  complete(artDate, source, fill = list(count = 0)) %>% 
  filter(!is.na(source))
...
### ngram
- 目的在於得知否定詞對於併校議題情緒的結果是否有很大的影響，以及找出特定詞彙的相關性。
...
#計算各來源情緒
source_sentiment_plot_data <- all_dates %>% 
  merge(source_sentiment_plot_data 
        ,by.x=c('artDate', "sentiment"),by.y=c('artDate', "sentiment_tag"),
        all.x=T,all.y=T) %>% 
  mutate(count = replace_na(count, 0)) %>% 
  complete(artDate, source, sentiment, fill = list(count = 0)) %>% 
  filter(!is.na(source))
...
- 除此之外，其他詞彙，像是:資源、校區、西子灣以及仁武，就是偏向討論中山的校地資源，推測可能是討論到校地不足才併校。
...
### topic 1 詞彙
#![](img/LDAvis_topic1.png)

### topic 2 詞彙
#![](img/LDAvis_topic2.png)

### topic 3 詞彙
#![](img/LDAvis_topic3.png)

### topic 4 詞彙
#![](img/LDAvis_topic4.png)

- 從以上的LDAvis分析的圖可以看到topic1與topic2蠻接近的，主要是談論中山校方對兩校併校的看法與評估以及學生的畢業證書及學校分數，所以topic2出現中山校長鄭英耀以及副校長蔡秀芬還有評估、公聽會等等的字眼；topic3則是講到高醫的董事會與校友會的關係，因為兩方長期處於對立的關係，校友會不滿董事會長期做的事，如果併校了可能會影響到校董的勢力；topic4則是與併校對兩校的未來的影響有關係，所以出現學校的「指標、分數、選系」以及企業「最愛」用的大學生等等關於學校的表現。
